...
class AutoClassifier:
    """
    Class AutoClassifier: AutoClassifier is a stand alone package that can be used to create a pipeline to
    perform binary classification almost instantly.
    It performs data cleaning, converts eligible features into categories, imputatation, feature selection,
    NLP, fits several classification models including a DNN model, performs hyperparameter tuning, and
    generates predictions with minimal user input.
    
     
    Initialization Parameters
    ----------
    orig_df: Pandas.DataFrame
        DataFrame with features and targets. Only two requirements for this is (i) the target vector should be
        binary and should be int or category type and should be the last column of the dataframe;
        (ii) All text columns should be concatenated into a single text feature vector.
    
    text_col: str, optional
        The name of the column with text. If text_col is provided, NLP methods will be applied to it.
        If None, then analysis is considered Non-NLP.
        Default None
    ...
    def __init__(self, orig_df, text_col=None):
        assert orig_df.iloc[:, -1].value_counts().shape[
                   0] == 2, "ERROR: The last column should be the target column and should be binary " \
                            "(True/False, 0/1, Success/Failure etc.) "
    ...
    def preprocess_block(self, cat_var_limit=10, bin_rep=1,
        ...
        bin_rep: int, optional
            style of integer representation for category variables, 0 for binary integer representation
            (OneHotEncode), 1 for 0 to nclass-1 representation
            Default 1
        ...
        use_feat_select: bool, optional
            If True then feature selection using LASSO for non-text columns is applied.
            Otherwise no feature selection will be performed
            Default True
        ...
        pre_proc_filename: str, optional
            Preprocessor trained on this dataset will be saved using this filename. Next dataset can be
            preprocessed by loading this file. Only activated if preprocessor was not previously loaded.
            Default: preproc.sav
        ...
        4 - Tuple
            If preprocessing was run from scratch (load_preproc is None). Tuple includes train test matrices
            and vectors (x_train,x_test,y_train,y_test)
        
        Numpy.Array
            If preprocess was loaded (load_preproc is Not None). This is X matrix which is ready to be passed
    ...
    def _preprocess_df(self, df_all, pre_proc_dict, preproc):
        ...
        preproc: str
            Filename of a previously trained preprocessor. Used here as an indicator,
            if None then preprocessing is being done for scratch.
        ...
        if not cat_col_df.empty:
            if bin_rep:
                ...
                df = pd.concat([pd.concat([df.iloc[:, :-1], temp_df], sort=False, axis=1), df.iloc[:, -1]], sort=False,
                               axis=1)
        ...
        assert df.select_dtypes(include=[
            'object']).empty, 'ERROR: Some columns could not be converted to category valuables and encoded. ' \
                              'Please segment non-numerical data columns (that is not the text column) or ' \
                              'increase cat_var_limit'
    ...
    def _feat_select(self, df, pre_proc_dict, pre_proc_filename, text_mat=None, test_size_var=0.3,
        ...
        use_feat_select: bool, optional
            If True then feature selection using LASSO for non-text columns is applied. Otherwise no
            feature selection will be performed
            Default True
        ...
        -------
        None
            Important features are saved after feature selection. Train test splits are saved as attributes
            self.x_train, self.x_test, self.y_train, self.y_test. Preprocessor is saved as a pickled file using
            filename pre_proc_filename.
        """
        ...
        if use_feat_select:
            ...
            try:
                ...
                print(
                    'WARNING: Lasso Coefficients all turned out to be 0 or could not be calculated. '
                    'Check your dataset or switch off feature selection.')
