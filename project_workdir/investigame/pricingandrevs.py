...
class PricingAndRevs:
    """
    Class PricingAndRevs: Used to scrape price histories, reviews, and game images for games in a
    dataframe of list of PS4 Games.
    
    Initialization Parameters
    ----------
    game_list: Pandas.DataFrame
        List of PS4 Games ideally obtained from GetWikiGameTable. DataFrame that contains a list of
        HTML ready search keywords for certain games.
    ...
    def get_all_price_histsandrev(self, col_name='TitlesHTML',
                                  ...
                                  pic_xpath=None):
        """
        Get price histories (as a DataFrame) and reviews from PSprices.com for a dataframe that includes a
        list of game search titles
        ...
        for title, link in zip(df['Titles'], links):
            (game_price_dict[title], link_tf, rev_val, game_ph) = \
                self._get_price_historyandrev_psprices(link,
                                                       game_name=title,
                                                       reg_string=pricehist_reg,
                                                       rev_xpath=review_xpath,
                                                       pic_xpath=pic_xpath)
        ...
        df['GameCard'] = df['GameCard'].fillna(
            'https://store.playstation.com/store/api/chihiro/00_09_000/container/US/en/19/UP4478-PCSE00649_00-2015072013150003/image?w=240&h=240&bg_color=000000&opacity=100&_version=00_09_000')
    ...
    def _get_price_historyandrev_psprices(self, addr, reg_string,
        ...
        """
        Get price histories (as a DataFrame) and reviews from PSprices.com for a
        certain game, given the right link.
        ...
        game_name: str, optional
            ...
            Only needed if there's a possibility that the website link might go into the search page
    ...
    def _get_reg_output(self, soup, reg_string):
        """
        Accepts a soup object, finds the right script tag with pricing details and returns the dates and price
        history using the find_reg function.
    ...
    def _convert_price_todataframe(self, reg_output):
        ...
        try:
            ...
            raise TypeError(
                'Error: Could not convert scraped price history table to dataframe. Check regex function and '
                'scraping mechanism.')
        try:
            price_history['Date'] = pd.to_datetime(price_history['Date'], infer_datetime_format=True, errors='coerce')
        except:
            print(
                'WARNING: Could not convert scraped price history dates into datetime format. Check scraping mechanism.')
        try:
            price_history['Price'] = pd.to_numeric(price_history['Price'], errors='coerce')
        except:
            raise TypeError(
                'ERROR: Could not convert scraped prices into float format. Check scraping mechanism and regex.')
    ...
    def _get_price_history_link(self, tree, game_name,
        ...
        addon: str, optional
            The main website address which will be appended to the game link extracted by the scraping
            algorithm to create a complete web link.
    ...
    def get_prepdf_withreg(self, days=None,
        ...
        publisher_medval: int, optional
            The capping value for medium throughput publishers. Publishers that have published more or equal to games
            than this will be considered High throughput.
            Default 18
            
        publisher_lowval: int, optional
            The capping value for low throughput publishers. Publishers that have published less games than or equal
            to this will be considered Low throughput.
            Default 3
    ...
    def _convert_genres(self, df, genre_map):
        """
        Maps many different Genres to a set of particular genres to reduce genre diversity.
        Mapping can be specified in a separate csv file.
    ...
    def _convert_catstrength(self, df, col_name, med_val, low_val):
        """
        Creates a new column that maps elements from an original column based on repetitions.
        Elements that are repeated very frequently will be call High, elements that are not repeated
        frequently will be called Low and in-between elements will be called Med.
